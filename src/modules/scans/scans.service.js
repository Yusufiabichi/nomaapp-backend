...
class ScansService {
  ...
  async createScan(userId, file, scanData) {
    ...
    // Trigger AI diagnosis asynchronously (don't wait)
    this.processDiagnosis(scan._id, scanData.language).catch(err => {
  ...
  async processDiagnosis(scanId, language = 'en') {
    ...
    try {
      ...
      // Call AI service with crop type
...
async getUserScans(userId, query = {}) { 
  const { page = 1, limit = 20, status, cropType } = query;
const filter = {
  userId,
  isDeleted: { $ne: true }
};
